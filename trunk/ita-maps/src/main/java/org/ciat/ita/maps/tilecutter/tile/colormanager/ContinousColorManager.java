...
public class ContinousColorManager extends ColorManager {
	 ...
	 * 
	 * @param rgbMin
	 *            valor de color minimo
	 * @param rgbMax
	 *            valor de color maximo
	 * @param min
	 *            valor minimo
	 * @param max
	 *            valor maximo
	 ...
	 *            valor de noData
	 */
	public ContinousColorManager(float[] rgbMin, float[] rgbMax, float min, float max, float NoData,
			Raster raster) {
	...
	public BufferedImage getScaleImage(String descripcion) {
		...
		for (int i = 0; i < nbStep; i++) {
			...
			graphics.drawString(formater.format(toWrite), margin / 2 - fWidth / 2 + (recWidth / nbStep) * i,
					recHeight + line - 2);
			graphics.drawLine(margin / 2 + (recWidth / nbStep) * i, recHeight, margin / 2
					+ (recWidth / nbStep) * i, recHeight + line / 4);
		}

		fWidth = fm.stringWidth(formater.format(Math.floor(this.getMax() * fact) / fact));
		graphics.drawString(formater.format(this.getMax()), margin / 2 + recWidth - fWidth / 2, recHeight
				+ line - 2);
		graphics.drawLine(margin / 2 + recWidth, recHeight, margin / 2 + recWidth, recHeight + line / 4);
		return image;
	}
