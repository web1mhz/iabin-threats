...
public class PortalManager {
	...
	public int insertGoodRecord(Record rec) {
		...
		query.append(") values("
				...
				+ rec.getLatitude()
				+ ",'"
				+ rec.getIso_country_code()
				+ "','"
				+ DataBaseManager.correctStringToQuery(getCanonical(rec
						.getNub_concept_id())) + "'");
		for (String var : rec.getVariables().keySet()) {
	...
	public int insertGoodRecordWithCanonical(Record rec) {
		...
		query.append(") values(" + rec.getId() + "," + rec.getNub_concept_id()
	...
	public synchronized List<Record> getRecords(int numRecords) {
		...
		try {
			while (contador < numRecords && !endRecords) {
				if (rs.next()) {
					regs.add(new Record(rs.getString("iso_country_code"), rs
							.getDouble("latitude"), rs.getDouble("longitude"),
							rs.getInt("nub_concept_id"), lastID = rs
									.getInt("id")));
					contador++;
				} else {
					endRecords = true;
	...
	public synchronized List<Record> getRecordsWithCountyLevelInfo(
		...
		try {
			while (contador < numRecords && !endRecords) {
				if (rs.next()) {
					regs.add(new Record(rs.getString("iso_country_code"), rs
							.getString("Pais"), rs.getString("Departamento"),
							rs.getString("Municipio"),
							rs.getDouble("latitude"),
							rs.getDouble("longitude"), rs.getInt("nub_concept_id"), rs
									.getString("canonical"), lastID = rs
									.getInt("id")));
					contador++;
				} else {
					endRecords = true;
	...
	public String getCountryISO(String pais) {
		...
		ResultSet rs = DataBaseManager.makeQuery(
				"select * from country_name where name= '"
						+ DataBaseManager.correctStringToQuery(pais) + "'",
				conx);
		try {
			if (rs.next()) {
