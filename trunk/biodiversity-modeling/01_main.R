...
sapply(sp, function(x) write.species.csv(records=x$records, class=x$class, req.points=pts.min, log="log.txt")) # the function write.species.csv is located in 001_write_species_csv.R
...
for (i in files)
{
 ...
 make.swd(sp_id=i, points=this.sp, ecoregions=eco, v.all=eco.values, no.background=no.background, dir.maxent=dir.maxent, env.var=env.var) # make.swd is located in the file 002_make_swd.R
 run.maxent(sp_id=i,max.ram=max.ram, dir.maxent=dir.maxent, no.repclicates=no.repclicates, replicate.type=replicate.type) # run.maxent is located in the file 003_run_maxent.R and zipWrite (provided by J. Ramirez)
}
