...
public class ColorManagerFactory {
	...
	/**
	 * Funcion que toma los valores del archivo de configuracion para determinar el color que se va a aplicar y la escala de color.
	 * 
	 * @param rasterID
	...
	public static ColorManager createColorManager(String rasterID, Raster raster) {
		...
		if (scaleType.equals(CONTINUOUS))
			return new ContinousColorManager(rgbMin, rgbMax, min, max, raster.getHeader().getNoData(), raster);

		if (scaleType.equals(FIXED)) {
			float umbral = PropertiesManager.getInstance().getPropertiesAsFloat(
					colorType + ".scale.threshold");
			FixedThresholdColorManager cm = new FixedThresholdColorManager(rgbMin, rgbMax, min, max, raster
					.getHeader().getNoData(), umbral, raster);
			cm.setThresholds(cm.createThresholds());
			return cm;
		}

		if (scaleType.equals(QUANTILE)) {
			float quantiles = PropertiesManager.getInstance().getPropertiesAsFloat(
					colorType + ".scale.quantiles");
			QuantilesColorManager cm = new QuantilesColorManager(raster, rgbMin, rgbMax, min, max, raster
					.getHeader().getNoData(), quantiles);
			cm.setThresholds(cm.createThresholds());
			return cm;
		}
